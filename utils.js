 ...
 *
 * @return {Function} Toggle function that will keep toggling the class on/off based on the condition passed to it. If
 * no condition is passed, the className will simply toggle on/off by itself
 *
 */
 ...
 */
export function getMetaQuery(key = '', value = '', index = 0) {
  const query = {}
  query[`meta_query[${ index }][key]`] = key
  query[`meta_query[${ index }][value]`] = value
  return query
}

export function getDisplayLocation({ location }) {
  return mapObject(location, (k, v) => v)
    .filter(v => v)
    .join(', ')
}

export function getDisplayIssueAreas({ issue_areas }) {
  return (issue_areas.length ? issue_areas : [])
    .slice(0, 3)
    .map(t => t.name)
    .join(', ')
}
...
/**
 * Check if an scrollable element has scrolled all the way down
 *
 * The +2 pixel is to account for subpixel rendering makining the difference
 * not match up when zoomed out
 *
 *  @param  {HTMLElement} el Element to check scroll for
 * @return {Boolean} Whether it has scrolled all the way or not
 */
export function elScrolledToBottom(el) {
  return el.scrollHeight - el.scrollTop <= el.offsetHeight + 2
}


export function findParentBySelector( elem, selector ) {
